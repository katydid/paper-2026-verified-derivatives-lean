...
def run' [DecidableEq φ] [Hashable φ] (f: Impl n φ α β) (t: Hedge.Node α): (List String × (Except String β)) :=
...
def runPopulatedMem [DecidableEq φ] [Hashable φ]
  (f: Impl n φ α β) (t: Hedge.Node α) (e: Regex.EnterMem.EnterMap (Hedge.Grammar.Symbol n φ)) (l: Regex.LeaveMem.LeaveMap (Hedge.Grammar.Symbol n φ))
  : EStateM.Result String (State n φ α) β :=
